...
# FastAPI does not have a global context with the Request object like Flask, using ContextVar to create one
...
class GCPLogFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord):
        ...
        # If a fastapi Request object is defined in the above ContextVar, fetch and add extra info, else write json log
        ...
        cloud_tasks_header = http_request_context.headers.get("X-CloudTasks-TaskName") or http_request_context.headers.get("X-AppEngine-TaskName")
        ...
        if trace_header:
            if "/" in trace_header:
                ...
                log_fields["logging.googleapis.com/trace"] = "projects/{gcp_project}/traces/{trace}" if gcp_project else trace
